...
export default {
	...
	methods: {
		...
		triallisten(index, row) {
			...
			const res = `http://rz2wdr1xy.hd-bkt.clouddn.com/${row.name}.mp3`;// http://xxx.xxx.com 是你在七牛云空间cname的域名
			this.sound.stop();
		...
		usebtnClick(index, row) {
			...
			const music = { name: row.name, url: `http://rz2wdr1xy.hd-bkt.clouddn.com/${row.name}.mp3`, path: null, isOnline: true, isUseBgm: true };
			this.$emit("music-change", music);
			this.dialogVisible = false;
		},
		// ========== 关闭对话框回调 ==========
